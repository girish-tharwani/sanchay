...
public class FinancialPlanningCalculator {
    ...
    public long computeEventForecast(MajorEvent event, PlanParameters params, LocalDate selfDob) {
        if (event.getType() == MajorEvent.EventType.ONE_TIME) {
            return event.getAmountPaise();
        }
        LocalDate upperBound = getRetirementDate(params, selfDob);
    ...
    public long computeMajorEventsKpi(PlanParameters params, LocalDate selfDob) {
        ...
        long forecast = params.majorEvents.stream()
                .mapToLong(event -> computeEventForecast(event, params, selfDob))
                .sum();
